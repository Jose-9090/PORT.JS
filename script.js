 window.addEventListener('load', function () {
        ...
        (function dragSeguirMouse() {
            var dragao = document.getElementById('dragonCursor');
            if (!dragao) return;
            ...
            (function animar() {
                ...
                dragao.style.left = atualX + 'px';
                dragao.style.top = atualY + 'px';
            ...
            function rugir() {
                ...
                dragao.classList.remove('roar');
                void dragao.offsetWidth;
                dragao.classList.add('roar');
            ...
            window.addEventListener('mousedown', rugir);

            var ctxRugido = null;
            function tocarRugido() {
                try {
                    if (!ctxRugido) {
                        ctxRugido = new (window.AudioContext || window.webkitAudioContext)();
                    }
                    if (ctxRugido.state === 'suspended') ctxRugido.resume();
                    var t = ctxRugido.currentTime;
                    var osc = ctxRugido.createOscillator();
                    var ganho = ctxRugido.createGain();
                    osc.type = 'sawtooth';
                    osc.frequency.setValueAtTime(160, t);
                    osc.frequency.exponentialRampToValueAtTime(70, t + 0.25);
                    ganho.gain.setValueAtTime(0.12, t);
                    ganho.gain.exponentialRampToValueAtTime(0.001, t + 0.3);
                    osc.connect(ganho);
                    ganho.connect(ctxRugido.destination);
                    osc.start(t);
                    osc.stop(t + 0.32);
                } catch (e) {}
            }
